...
export function activate(context: ExtensionContext) {
  ...
  const config = workspace.getConfiguration("openInForkButton");

  if (config.get("iconButton") === false) {
    myButton.text = "$(fork-logo) Open in Fork";
  } else {
    myButton.text = "$(fork-logo)";
  }
    ...
    if (
      ...
      cp.exec(
        ...
        (err: any, stdout: any, stderr: any) => {
          if (err) {
            window.showErrorMessage("WSL Error: " + err);
            return;
          }
          ...
          cp.exec("lsb_release -a", (err: any, stdout: string, stderr: any) => {
            ...
            const distroLine = stdout
              .split("\n")
              .find((line) => line.startsWith("Distributor ID:"));
            if (distroLine) {
              const distroName = distroLine?.split(":")?.pop()?.trim();
              const winRootPath = `\\\\\\\\wsl.localhost\\\\${distroName}${rootPath.replace(
                /\//g,
                "\\\\"
              )}`;
